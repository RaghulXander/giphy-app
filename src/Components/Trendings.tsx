...
export default function Trendings(){
  ...
  const { apiData }: { apiData: TApiData | null; } = useApi(trendingUrl, setLoading, useIniniteScroll)
  ...
  return (
    ...
    <div className="flex items-center mx-[10%] py-[15px] border-b-[2px]">
      <IconComponent Icon={MdOutlineTrendingUp} className="text-[50px] text-[#90d8e8]" />
    ...
    {!loading &&
      <div className="flex flex-col justify-center items-center" >
        ...
        <button className="bg-[#6157ff] rounded-[2px] h-[40px] w-[120px] my-[15px] text-white cursor-pointer " onClick={() => loadMore()}>Load More</button>
      </div>
    }
      </div>
      </>)
